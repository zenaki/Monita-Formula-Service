        ...
        if (obj == "REDIS") {
            ...
            if (object.value(obj).isArray()) {
                ...
                foreach (const QJsonValue & v, array) {
                    ...
                    result.append(v.toObject().value("ROOT_FORMULA").toString());
                ...
                result.append(value.toObject().value("ROOT_FORMULA").toString());
...
void config::write(QJsonObject &json) const //Default
{
    ...
    configObject["ROOT_FORMULA"] = "/var/www/html/talisa";
